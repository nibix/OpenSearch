 ...
 *   <li>Reads the {@code key_metadata} JSON from the Parquet footer via
 *       {@link NativeBridge#readParquetKeyMetadata}.</li>
...
public class DataFusionEncryptedQueryExecutionTests extends OpenSearchTestCase {
    ...
    private ReaderHandle encryptedReader() throws Exception {
        ...
        byte[] keyMetadataBytes = NativeBridge.readParquetKeyMetadata(filePath);
    ...
    private List<Object[]> executeQuery(ReaderHandle reader, String sql) {
        ...
        long streamPtr = asyncCall(
            listener -> NativeBridge.executeQueryAsync(reader.getPointer(), TABLE, substraitBytes, runtimeHandle.get(), listener)
        );
