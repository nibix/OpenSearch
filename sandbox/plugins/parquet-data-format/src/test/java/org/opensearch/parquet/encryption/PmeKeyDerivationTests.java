...
public class PmeKeyDerivationTests extends OpenSearchTestCase {
    ...
    private static final int DATA_KEY_BYTES = PmeKeyDerivation.DATA_KEY_BYTES;     // 32
    private static final int MESSAGE_ID_BYTES = PmeKeyDerivation.MESSAGE_ID_BYTES;  // 16
    private static final int FOOTER_KEY_BYTES = PmeKeyDerivation.FOOTER_KEY_BYTES;  // 16 (AES-128 limit)
    ...
    public void testDeriveFooterKeyOutputLength() {
        byte[] key = PmeKeyDerivation.deriveFooterKey(dataKey((byte) 0xAA), messageId((byte) 0xBB));
        assertEquals("footer key must be FOOTER_KEY_BYTES long", FOOTER_KEY_BYTES, key.length);
    }
