...
# while the key provider plugin is absent must be rejected (the plugin that
# registers the setting and the key provider are both gone).
...
if [[ "${new_enc_status}" == "400" || "${new_enc_status}" == "500" ]]; then
    ...
    # If the setting is simply unknown (plugin removed), OpenSearch may also
    # return 400 with "unknown setting".
    fail "T9-B/new-encrypted-index-rejected-without-kms" \
        "expected 4xx/5xx, got HTTP ${new_enc_status}"
fi
